...
function viewStudentStats(id, toggle) {
	...
	console.log("viewStudentStats finished");
}
...
function changeColor(color) {
	...
	console.log("changeColor finished");
}
...
function loadStatsPage() {
	...
	for(var i = 1, row; row = tab.rows[++i];) {
		if(parseInt(row.cells[2].innerHTML, 10) > parseInt(row.cells[3].innerHTML, 10)) {
			row.cells[4].innerHTML = "Home";
			wins++;
		} else {
			row.cells[4].innerHTML = "Away";
			losses++;
		}
	}
	...
	console.log("loadStatsPage finished");
}
...
function loadPlayersPage() {
	...
	for(var i = 0; i < players.length; i++) {
		dropOptions += "<a class='dropdown-item' href='#' onclick='switchPlayers("+i+")'>"+players[i].name+"</a>"
	}
	document.getElementById("player_selector").innerHTML=dropOptions;
}
...
function switchPlayers(playerNum) {
	...
	document.getElementById("player_img").innerHTML = "<img url="+players[playerNum].img+" alt="+players[playerNum].alt+">";
